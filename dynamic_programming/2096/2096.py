...
n = int(input())
for i in range(n):
    newline = tuple(map(int, input().split()))
    grid_max = [newline[0] + max(grid_max[0:2]), newline[1] + max(grid_max[0:3]), newline[2] + max(grid_max[1:3])]
    grid_min = [newline[0] + min(grid_min[0:2]), newline[1] + min(grid_min[0:3]), newline[2] + min(grid_min[1:3])]
